...
function setupReveal() {
  ...
  /* An IntersectionObserver only reports threshold *crossings*. An element that
     was below the fold and is now above it — because the reader pressed End,
     followed an anchor deep into the page, dragged the scrollbar, or came back
     to a page the browser restored the scroll position of — never intersected,
     so it never fires and stays at opacity 0 permanently.

     On a page like People, where eight sections sit below the first screen,
     that is most of the page invisible. Motion must never be the reason
     content cannot be read, so this sweep is the backstop: anything at or above
     the fold is revealed whether or not the observer saw it happen.

     Reads are batched before writes, it is rAF-throttled, and it removes itself
     the moment nothing is left pending — so it costs nothing on a page that has
     finished revealing. */
  const sweep = () => {
    frame = 0;
    const fold = window.innerHeight;
    const arrived = pending.filter((el) => el.getBoundingClientRect().top < fold);
    if (arrived.length) {
